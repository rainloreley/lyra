        ...
        try {
            ...
            for (var i = 0; i < jsondata.devices.length; i++) {
                ...
                if (typeof device.device === 'string') {
                    const deviceConfig = dmxDevices.findIndex(
                        (e) => e.uuid === device.device
                    );
                    if (deviceConfig === -1 || deviceConfig === undefined) {
                        console.error(
                            `ERROR: Project cannot be loaded because the config file for ${device.name} is missing (id ${device.device})`
                        );
            ...
            <div className="overflow-hidden w-screen text-black h-screen">
                ...
                >
                    <div className={'flex flex-col text-center'}>
                        ...
                        <button
                            className={
                                'bg-blue-500 rounded-xl mt-2 text-white p-2 align-middle'
                            }
                            onClick={() => {
