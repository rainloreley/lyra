...
const Dashboard: NextPage = () => {
    ...
    return (
        <div className="overflow-hidden w-screen text-black h-screen">
            ...
            >
                <div
                    className="flex dark:bg-gray-800 bg-gray-200 h-16  border-b-2 border-gray-300 dark:border-gray-800 justify-between shadow-lg rounded-xl m-2">
                    <div className={'flex'}>
                        ...
                        {projectManager.currentProject?.devices.length > 0 ? (
                            <div className={'m-2'}>
                                {projectManager.currentProject?.devices.map((e) => (
                                    <button
                                        key={e.start_channel}
                                        className={`dark:text-white justify-self-center self-center p-2 m-2 w-40 bg-gray-100 dark:bg-gray-800 rounded-xl shadow-xl ${selectedDevices.filter((_filter) => _filter.id === e.id).length > 0 ? "border border-yellow-300" : ""}`}
                                        onClick={(clickEvent) => {
