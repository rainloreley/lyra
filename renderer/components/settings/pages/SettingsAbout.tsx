...
const SettingsAbout: FunctionComponent = ({}) => {
    ...
    return (
        <div className={`w-full h-screen overflow-y-scroll overflow-x-hidden flex justify-center items-center ${styles.noscrollbar}`}>
            <div className={" text-center items-center flex flex-col"}>
                <img src={"/images/logo.png"} width={150} />
                <button className={"text-3xl font-bold"} onClick={() => {
                    easterEggClicked()
                }}>Lyra v{appVersion}</button>
                <p className={"italic text-gray-500 dark:text-gray-400 mt-1"}>Open Source DMX Light control software</p>
                <a className={"underline text-blue-500 mt-2"} href={"https://github.com/rainloreley/lyra"} target={"_blank"}>GitHub</a>
            </div>
        </div>
    )
}
